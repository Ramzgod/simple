...
\t|_______/[___]   \'-;__/[_____][_____]'.__.'[___]    	
			{m}•{k}•{h}•{sir} Author : XyzonXD {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def menu(id):
	...
	cetak(nel('\tSelamat datang Di Layanan Kami Bossku '))
	print(f'>> Your Idz : {id}')
	print(f'>> Your Ip  : {ip}')
	print('')
	...
	print('>> 5. Hasil Crack  ')
	print('>> 0. Keluar       ')
	_____xyzon__xd_____ = input('\n>> Pilih : ')
	if _____xyzon__xd_____ in ['1']:
		dump_massal()
	elif _____xyzon__xd_____ in ['2']:
		dump_follower()
	elif _____xyzon__xd_____ in ['3']:
		grup()
	elif _____xyzon__xd_____ in ['4']:
		crack_file()
	elif _____xyzon__xd_____ in ['5']:
		result()
	elif _____xyzon__xd_____ in ['0']:
